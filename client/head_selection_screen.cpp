...
HeadSelectionScreen::HeadSelectionScreen(SDL2pp::Renderer& renderer,
                                         const std::string& assetsPath):
        renderer(renderer),
        background(renderer,
                   SDL2pp::Surface(assetsPath + "/Pantallas/Seleccion_personaje.png")),
...
SDL2pp::Rect HeadSelectionScreen::pngToScreen(int x, int y, int w, int h) const {
    return SDL2pp::Rect(bgX + static_cast<int>(x * scaleX),
                        bgY + static_cast<int>(y * scaleY),
...
void HeadSelectionScreen::handleMouseClick(int mouseX, int mouseY) {
    ...
    if (mouseX >= jugar.x && mouseX < jugar.x + jugar.w &&
        mouseY >= jugar.y && mouseY < jugar.y + jugar.h) {
        confirmed = true;
        running   = false;
        return;
    }
...
void HeadSelectionScreen::render() {
    ...
    renderer.Copy(background, SDL2pp::NullOpt,
                  SDL2pp::Rect(bgX, bgY, displayW, displayH));
