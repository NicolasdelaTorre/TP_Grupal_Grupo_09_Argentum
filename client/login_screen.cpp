...
LoginScreen::LoginScreen(SDL2pp::Renderer& renderer):
        renderer(renderer),
        fontTitle("common/Alegreya-Sans-AO-Bold.ttf", 28),
...
void LoginScreen::handleTextInput(const SDL_TextInputEvent& text) {
    if (inputText.size() >= MAX_USERNAME)
        return;
...
void LoginScreen::render() {
    ...
    // ── Título ────────────────────────────────────────────────
    {
        ...
        renderer.Copy(tex, SDL2pp::NullOpt, SDL2pp::Rect(cx - w / 2, 80, w, h));
    }
    ...
    // ── Subtítulo ─────────────────────────────────────────────
    {
        ...
        auto [w, h] = tex.GetSize();
        renderer.Copy(tex, SDL2pp::NullOpt, SDL2pp::Rect(cx - w / 2, 130, w, h));
    }
    ...
    if (!displayText.empty()) {
        ...
        int textX = boxX + 12;
        renderer.Copy(tex, SDL2pp::NullOpt, SDL2pp::Rect(textX, textY, std::min(w, boxW - 24), h));
    }
    ...
    if (inputText.empty()) {
        ...
        auto [w, h] = tex.GetSize();
        renderer.Copy(tex, SDL2pp::NullOpt, SDL2pp::Rect(boxX + 12, boxY + (boxH - h) / 2, w, h));
    }

    // ── Hint Enter ────────────────────────────────────────────
    {
        std::string hint = inputText.empty() ? "Escribí tu nombre y presioná Enter" :
        ...
        auto [w, h] = tex.GetSize();
        renderer.Copy(tex, SDL2pp::NullOpt, SDL2pp::Rect(cx - w / 2, 250, w, h));
    }

    // ── Límite de caracteres ──────────────────────────────────
    {
        std::string counter = std::to_string(inputText.size()) + "/" + std::to_string(MAX_USERNAME);
...
SDL2pp::Texture LoginScreen::makeText(SDL2pp::Font& font, const std::string& text,
                                      const SDL_Color color) const {
    return SDL2pp::Texture(renderer, font.RenderUTF8_Blended(text, color));
}
