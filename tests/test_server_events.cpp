...
template <typename T>
std::unique_ptr<T> roundTrip(const ServerEvent& src, ProtocolLoopback& link) {
...
TEST(ServerEventTest, loginFailOpcodeOnly) {
    ...
    auto recv = roundTrip<OpcodeOnlyEvent>(original, link);
...
TEST(ServerEventTest, loginOkRoundTrip) {
    ...
    auto recv = roundTrip<LoginOkEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, loginOkCoordenadasNegativas) {
    ProtocolLoopback link;
...
TEST(ServerEventTest, mapVacioRoundTrip) {
    ProtocolLoopback link;
    MapEvent original(10, 20, {});
    auto recv = roundTrip<MapEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, mapConCeldasYObstaculos) {
    ProtocolLoopback link;
    std::vector<MapCellData> cells = {{1, 0, false}, {2, 5, true}, {3, 0, false}};
    std::vector<MapObstacleData> obstacles = {{2, -3, 4, 2, 1}, {5, 10, 10, 3, 2}};
    ...
    EXPECT_EQ(recv->getObstacles()[0].type, 2);
...
TEST(ServerEventTest, newPlayerRoundTrip) {
    ...
    auto recv = roundTrip<NewPlayerEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, moveRejectedRoundTrip) {
    ...
    auto recv = roundTrip<MoveRejectedEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, playerMovedRoundTrip) {
    ...
    auto recv = roundTrip<PlayerMovedEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, playerDisconnectedRoundTrip) {
    ...
    auto recv = roundTrip<PlayerDisconnectedEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, statsRoundTrip) {
    ...
    auto recv = roundTrip<StatsEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, statsGoldMaximo) {
    ProtocolLoopback link;
...
TEST(ServerEventTest, attackResultHit) {
    ...
    auto recv = roundTrip<AttackResultEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, inventoryUpdateRoundTrip) {
    ...
    auto recv = roundTrip<InventoryUpdateEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, inventoryUpdateVacio) {
    ProtocolLoopback link;
...
TEST(ServerEventTest, playerEquippedRoundTrip) {
    ...
    auto recv = roundTrip<PlayerEquippedEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, newNpcVivoYMuerto) {
    {
        ProtocolLoopback link;
        NewNpcEvent original(50, 12, 34, 3, true);
        auto recv = roundTrip<NewNpcEvent>(original, link);
        ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, npcMovedRoundTrip) {
    ...
    auto recv = roundTrip<NpcMovedEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, npcDiedRoundTrip) {
    ...
    auto recv = roundTrip<NpcDiedEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, npcRespawnedRoundTrip) {
    ...
    auto recv = roundTrip<NpcRespawnedEvent>(original, link);
    ASSERT_NE(recv, nullptr);
    EXPECT_EQ(recv->getX(), 33);
    EXPECT_EQ(recv->getY(), 44);
}

TEST(ServerEventTest, chatBroadcastDeJugador) {
    ProtocolLoopback link;
    ChatBroadcastEvent original(5, "Tomas", "che, vamos al dungeon");
    auto recv = roundTrip<ChatBroadcastEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, chatBroadcastDelSistema) {
    ProtocolLoopback link;
...
TEST(ServerEventTest, playerDiedRoundTrip) {
    ...
    auto recv = roundTrip<PlayerDiedEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, playerRevivedRoundTrip) {
    ...
    auto recv = roundTrip<PlayerRevivedEvent>(original, link);
    ASSERT_NE(recv, nullptr);
    EXPECT_EQ(recv->getId(), 13);
...
TEST(ServerEventTest, itemDroppedNormal) {
    ...
    auto recv = roundTrip<ItemDroppedEvent>(original, link);
    ASSERT_NE(recv, nullptr);
...
TEST(ServerEventTest, itemDroppedOro) {
    ProtocolLoopback link;
...
TEST(ServerEventTest, itemPickedUpRoundTrip) {
    ...
    auto recv = roundTrip<ItemPickedUpEvent>(original, link);
    ASSERT_NE(recv, nullptr);
    EXPECT_EQ(recv->getDropId(), 200);
}

TEST(ServerEventTest, deserializeOpcodeInvalidoTira) {
    ProtocolLoopback link;
